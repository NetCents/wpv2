...
class NC_Widget_Payment_Gateway extends WC_Payment_Gateway {
  ...
  public function __construct() {
    ...
    function nc_gateway_callback_handler() {
     	if ($_POST['signature'] && $_POST['signing'] && $_POST['data']) {
        $signature = $_POST['signature'];
        $exploded_parts = explode(",", $signature);
        $timestamp = explode("=", $exploded_parts[0])[1];
        $signature = explode("=", $exploded_parts[1])[1];
        $decoded_data = json_decode(base64_decode($_POST['data']), true);
        $hashable_payload = $timestamp . '.' . $_POST['data'];
        $hash_hmac = hash_hmac("sha256", $hashable_payload, $_POST['signing']);
    ...
    function nc_gateway_request_access ( $order, $order_id ) {
      ...
      $encrypted_string = wp_remote_post('https://gw-stage.net-cents.com/api/v1/widget/encrypt/', array(
      		'method' => 'POST',
    			'body' => array(
	    ...
	    $response = wp_remote_get('https://gw-stage.net-cents.com/api/v1/widget/authorization?widget_id=' . $merchant_id . '&data=' . $decoded_body->token . '&origin=' . $_SERVER["HTTP_ORIGIN"]);
	...
	function process_payment ( $order_id ) {
        ...
        $access = $this->nc_gateway_request_access($order, $order_id);
        if (gettype($access) == 'string') {
            try {
                return array(
                    'result' => 'success',
                    'redirect' => 'https://gw-stage.net-cents.com/merchant/widget?data=' . $access . '&widget_id=' . $this->merchant_id
                );
            } catch (Exception $ex) {
